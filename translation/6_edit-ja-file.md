...
- `sourceCommit` のハッシュ値は、編集作業時点の content リポジトリの当該記事ページのファイル（英語版）の最新コミットハッシュ値（`git log` で確認できる。上の Entity の場合は、 `git log ~/path/to/mdn_repos/content/files/en-us/glossary/entity/index.md` ）を記述します。
